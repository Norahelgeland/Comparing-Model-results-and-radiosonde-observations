...
bufr_extract.ipynb is used to place the data from the bufr files(at each timestep) in csv files corresponding to each radiosonde sounding. The date has to be changed to retrieve the correct bufr data. Also the desired area has to be defined.
...
**Interpolate_exner_era5.py** does the same as **Interpolate_exner_arome.py** but is adapted to era5 and does does not include interpolation in time. This is just because the temporal resolution of the era5 files is too large. It can be added again if there are more timesteps availeble.
